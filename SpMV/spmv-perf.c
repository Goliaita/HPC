 ...
 * m: Is the row size of A
 **/
float *solutionSpMV(float *values, int *colIndex, int *rowIndex, float *x, int m, int iteration) {
    ...
    float val = (values[iteration] * x[colIndex[iteration]]);
    solution[rowIndex[iteration]] += (values[iteration] * x[colIndex[iteration]]);

    return solution;
}


double simulation(int m, int n, int i, float *values, int *colIndex, int *rowIndex, float *x){
    ...
    float *y = solutionSpMV(values, colIndex, rowIndex, x, m, i);        // Solves SpMV using the compressed information
...
int main(int argc, char *argv[]){
   ...
   // Arrays containing the compressed information of A
    ...
    for(int i = 0; i < nnz; i++){
        elaps += simulation(m, n, i, values, colIndex, rowIndex, x);   		
    }
